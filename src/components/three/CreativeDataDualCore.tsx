...
const CreativeDataDualCore: React.FC<CreativeDataDualCoreProps> = ({ className = "" }) => {
  ...
  useEffect(() => {
    ...
    const lightData = new THREE.PointLight(0x06b6d4, 4, 20); // Cyan data glow
    ...
    // Store original positions for vertex wave deformation
    ...
    const animate = () => {
      ...
      mainGroup.rotation.y = currX;
      mainGroup.rotation.x = -currY;
  ...
  return (
    <div className={`relative flex flex-col items-center justify-center ${className}`}>
      ...
      <div 
        ref={containerRef} 
        className="w-full h-[320px] md:h-[360px] cursor-grab active:cursor-grabbing"
      />

      {/* Floating Indicators */}
      <div className="flex items-center justify-between w-full px-6 -mt-4 z-10 text-xs font-mono">
        <div className="flex items-center gap-2 text-purple-400 bg-purple-950/40 px-3 py-1.5 rounded-full border border-purple-500/20 backdrop-blur-md">
        ...
        <div className="flex items-center gap-2 text-cyan-400 bg-cyan-950/40 px-3 py-1.5 rounded-full border border-cyan-500/20 backdrop-blur-md">
