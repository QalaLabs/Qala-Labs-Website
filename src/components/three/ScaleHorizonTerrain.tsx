...
const MILESTONES = [
  ...
  { label: '\u20B950L/mo', status: 'Inflection', x: -1.5, z: -4, color: 0x06b6d4 },
  { label: '\u20B91Cr/mo', status: 'Scale Phase', x: 1.5, z: -6, color: 0x10b981 },
  { label: '\u20B95Cr/mo', status: 'Category Leader', x: 4, z: -8, color: 0xf59e0b },
];

const ScaleHorizonTerrain: React.FC<ScaleHorizonTerrainProps> = ({ className = "" }) => {
  ...
  useEffect(() => {
    ...
    const dirLight = new THREE.DirectionalLight(0x06b6d4, 3);
  ...
  return (
    <div className={`relative rounded-3xl border border-white/10 bg-[#06070d] p-5 overflow-hidden select-none ${className}`}>
      {/* 3D Canvas */}
      <div 
        ref={containerRef} 
        className="w-full h-[280px] md:h-[340px] cursor-grab active:cursor-grabbing rounded-2xl overflow-hidden"
        ...
        <div className="grid grid-cols-4 gap-2">
          {MILESTONES.map((m, idx) => (
            <button
              ...
              className={`p-2 rounded-xl text-left transition-all font-mono border ${
                scaleLevel === idx + 1
            ...
            >
              ...
              <div className="text-[9px] text-slate-400 truncate">{m.status}</div>
            </button>
          ))}
        </div>
      </div>
